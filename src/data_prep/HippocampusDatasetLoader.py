...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    for f in images:
        ...
        # ANSWER: Because lable takes values of class and cannot be floating point and loss calculation 
        # cross entropy loss which requires random variables of integer class
        #print(f"Image should be of new shape {image.shape[0],y_shape, z_shape}")
        #print(image.shape,label.shape)
        #input("Pause here 4")

        out.append({"image": image, "seg": label, "filename": f})
        #print(f"seg: {label} filename : {f} mean:{np.mean(image)}")
